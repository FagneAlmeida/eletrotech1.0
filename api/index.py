...
# Inicialização Blindada (Evita crash no restart da função)
...
# --- ROTAS CORRIGIDAS (SEM PREFIXO /API NO CÓDIGO) ---

@app.get("/orcamentos")
async def listar(x_api_key: str = Header(None)):
    if x_api_key != API_KEY_SECRET: raise HTTPException(status_code=401)
    # Busca simples para garantir a volta do sistema
    docs = db.collection("orcamentos").limit(50).stream()
